var AppProcess = (function () {
    ...
    function removeVideoStream(rtp_vid_senders) {
        if (videoCamTrack) {
            videoCamTrack.stop();
            videoCamTrack = null;
            var video = document.querySelector('#locaVideoPlayer');
    ...
    async function videoProcess(newVideoState) {
        ...
        try {
            ...
            if (vstream && vstream.getVideoTracks().length > 0) {
                ...
                if (videoCamTrack) {
                    var video = document.querySelector('#locaVideoPlayer');
                    video.srcObject = new MediaStream([videoCamTrack]);
...
var MyApp = (function () {
    ...
    $(document).on("click", ".end-call-wrap", function () {
        $(".top-box-show").css({
            "display": "block"
        }).html('<div class="top-box align-vertical-middle profile-dialogue-show"> <h4 class="mt-3" style="text-align: center; color: white;">Leave Meeting</h4><hr> <div class="call-leabe-cancel-action d-flex justify-content-center align-items-center w-100"> <a href="/action.html"><button class="call-leave-action btn btn-danger mr-5">Leave</button></a> <button class="call-cancel-action btn btn-secondary">Cancel</button> </div> </div>')
    });
    ...
    $(document).on("click", ".stop-record", function () {
        $(this).removeClass().addClass("start-record btn-danger text-dark").text("Start Recording");
        mediaRecorder.stop();
    });
